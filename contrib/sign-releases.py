...
To create a keystore run the following command:
    ...
    mkdir ~/.jks && keytool -genkey -v -keystore ~/.jks/keystore \
        -alias electrum.polis.org -keyalg RSA -keysize 2048 \
        -validity 10000
...
Manual signing:
    ...
    jarsigner -verbose \
        ...
        -sigfile polis-electrum \
        -keystore ~/.jks/keystore \
        Electrum_POLIS-3.0.6.1-release-unsigned.apk \
        electrum.polis.org
...
Manual zip aligning:
    ...
    android-sdk-linux/build-tools/27.0.3/zipalign -v 4 \
        Electrum_POLIS-3.0.6.1-release-unsigned.apk \
        Electrum_POLIS-3.0.6.1-release.apk
...
SDIST_NAME_PATTERN = re.compile('^Electrum-POLIS-(.*).tar.gz$')
SDIST_DIR_TEMPLATE = 'Electrum-POLIS-{version}'
PPA_SOURCE_NAME = 'electrum-polis'
...
UNSIGNED_APK_PATTERN = re.compile('^Electrum_POLIS-(.*)-release-unsigned.apk$')
SIGNED_APK_TEMPLATE = 'Electrum_POLIS-{version}-release.apk'
